...
//   РАСХОДЫ ТТ:   cat=3144 org=2 (все: pm>0 + pm=0 НДС в платежах поставщикам)
...
function calc(pls){
  ...
  pls.forEach(function(r){
    ...
    // НДС по расходам ТТ (cat=3144, org=2) — включать всегда, независимо от pm
    var isTtExpNds=(cat===3144&&oid===2);

    // Правило включения: pm>0 и pm≠980, ИЛИ pm=0 при зеркальном переводе ВСИП↔ТТ или НДС-расходах ТТ
    if(pm===980) return;
    if(pm===0 && !isVsipTransfer && !isTtIncoming && !isTtExpNds) return;
